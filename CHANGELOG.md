...
> 🧹 **监控范围收敛：勾选生效 + 移除手动名单 + 默认 5 分钟**。依据用户反馈清理三处长期困惑点。

### 变更

- **「特别关注」复选框真正生效**：后台监控对象改为 `special.filter(u => selectedSet.has(u.id))`，只监控设置页勾选的人；**勾选为空 = 不监控任何人**（用户在设置页明确要求）。读取分组后默认全选（避免「读出来却不监控」），用户可取消个别。
- **删除手动名单（`manualUsers`）**：移除整条手动监控路径（`background.js` 的 `parseManualUsers` / `manualOnly` 补抓块、`options.js` 读写、`options.html` 输入框）。监控源唯一且明确 = 「特别关注分组里勾选的人」，不再有「二选一」歧义。
- **默认与文案统一为 5 分钟**：设置页 `interval` 默认值由 2 → 5，提示文案改为「默认 5 分钟」（`scheduleAlarm` 下限 3 分钟不变，仍为风控保护）。
- 测试：新增「勾选子集生效」「勾选为空仍不监控」断言，全套 **65 PASS / 0 FAIL**。

## [1.5.8] - 2026-08-22

> 🛡️ **兜底深扫：保证特别关注"绝不漏人"**。v1.5.7 合并流只抓最新 2 页，若特别关注帖被大量关注对象的新帖挤出视野，理论上可能延迟甚至漏抓。v1.5.8 在合并流（负责"即时"）之外，加一层**轮转逐人直查 `user_timeline`**（每轮只查 2 人、串行铺开、跨轮持久化进度），30 人分组下每人约每 75 分钟被直查一次——合并流漏掉的，深扫一定补上。每轮总请求仅约 4 个（2 合并 + 2 深扫），远低于原 30 个/轮，不会重新触发 WAF。

### 变更

- 新增 `deepScanState`（存 `storage.deepScan`）轮转指针，`checkOnce` 每轮对特别关注里 `DEEP_PER_CYCLE=2` 人做原有 `fetchUserFresh` 直查，结果与合并流合并进 `newAll`/`lastIds`/`perUser`。
- 深扫命中 `[BLOCKED]|429` → 熔断并中止本轮（与合并流同处理）。
- 测试：全套 **61 PASS / 0 FAIL**。

## [1.5.7] - 2026-08-22

> 🔥 **根治雪球 WAF「访问被阻断」——从「事后熔断」改为「事前预防」**。旧方案（v1.5.4 的熔断）是**检测到阻断页后再退避**，但雪球 WAF 是**即时 + 会话/IP 级**封锁，请求一打出去就被拦、且连带污染手动浏览雪球的标签页，熔断来得太晚、封禁早已造成，所以体感上"完全没用"。根因是**每轮对特别关注里每个人各打一次 `user_timeline.json`**（30 人 = 每轮 30 个请求），这才是踩风控的源头。

### 变更

- **取数路径重写**：特别关注改为**一次拉合并信息流 `home_timeline.json`（2 页）**，本地按 `user_id` 过滤出特别关注的新帖。请求量从 **N 个/轮 → 1~2 个/轮**（30 人约降 30 倍），从源头让 WAF 触发不了。
- **`home_timeline` 双变体兜底**：先试 `v4/...` 再试旧版 `...`，任一拿到即止，避免单路径 404 取空。
- **旧熔断逻辑降级为兜底安全网**：`rlTrip/rlReset` 保留，仅在极端情况（如手动狂刷雪球触发风控）才生效，正常轮询基本用不到。
- **手动名单**里不在特别关注分组内的用户仍走原有的逐人 `user_timeline`（请求少、串行铺开），不受影响。
- 新帖判定（按用户 `max id` + 初始化首轮不通知）、通知/弹窗/企微推送链路**完全不变**。
- 测试：新增合并流取数路径断言，全套 **61 PASS / 0 FAIL**。
